...
if __name__ == "__main__":
    ...
    model = dnn_py.nn_model(6, 20, 1, act, 10)
    model.to(device)
    loss_fn = torch.nn.MSELoss()
    n_epoch = 150
    opt = torch.optim.Adam(params=model.parameters(), lr=0.001)
    [train_loss_history_dnn, val_loss_history_dnn, epoch] = dnn_py.train(model, X_train, y_train, X_val, y_val, n_epoch, loss_fn, opt, 1e-6, 1e-4, verbose=True)
    ...
    # model_d = dnn_py.NN(6, 10, 1, 100)
    # opt_d = torch.optim.Adam(params=model.parameters(), lr=0.001)
    # [train_loss_history_dnn, val_loss_history_dnn, epoch] = model_d.train(X_train, y_train, X_val, y_val, n_epoch, loss_fn, opt_d, 1e-6, 1e-4, verbose=True)
