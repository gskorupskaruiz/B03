...
def maxima(data, val):
    maxima = []
    location = []
    for i in range(1, len(data)-val):
        if data[i] > data[i+val] and data[i] > data[i-val]:
            maxima.append(data[i])
            location.append(i)
    
    return maxima, location
...
# predict_hybrid_cv = predict_hybrid
# predict_lstm_cv = predict_lstmcnn

predictions_hybrid = model_hybrid(test_data_h).to('cpu').detach().numpy()
predict_hybrid = predictions_hybrid.squeeze(2).squeeze(1) * time_std_h + time_mean_h
test_truth_h = test_truth_h.squeeze(2).to('cpu').detach().numpy() * time_std_h + time_mean_h
predictions_lstmcnn = model_data(test_data_).to('cpu').detach().numpy()
predict_lstmcnn = predictions_lstmcnn.squeeze(2) * time_std_ + time_mean_
predict_hybrid_cv = predict_hybrid
...
plt.ylabel('Time to discharge (seconds)', fontsize=12)
plt.legend()
plt.style.use('seaborn-darkgrid')
plt.show()

print(f'moving on to max predictions')
max_ttd_truth, location = maxima(test_truth_h, 1)
max_ttd_hybrid = []
for i in location:
    max_ttd_hybrid.append(predict_hybrid[i+3])
max_ttd_data = []
for i in location:
    max_ttd_data.append(predict_lstmcnn[i+3])

# print(f'shape of truth data is {test_truth_h.shape}, shape of hybrid data is {predict_hybrid.shape}, shape of data-driven data is {predict_lstmcnn.shape}')

# print(f'shape of truth is {len(max_ttd_truth)}, shape of hybrid *is {len(max_ttd_hybrid)}, shape of data-driven is {len(max_ttd_data)}')

plt.plot(location, max_ttd_truth, label='Ground truth')
plt.plot(location, max_ttd_hybrid, label= 'Hybrid model')
plt.plot(location, max_ttd_data, label='Data-driven model')
plt.xlabel("Cycle instance (-)")
plt.ylabel("Maximum time to discharge for a cycle")
plt.legend()
plt.show()

# with open('data_alambda_plotssss.csv', 'w') as f:
#     csv_writer = csv.writer(f)
#     csv_writer.writerow(['Predict Hybrid', 'Predict Data Driven', 'Test Truth'])
#     csv_writer.writerows([predict_hybrid_cv, predict_lstm_cv, test_truth_h])
...
# plt.figure()
# plt.plot(predictions_plot, label='Model predictions', linewidth=1, color='orchid')
