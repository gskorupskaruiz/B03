...
def lr_opt(model, X_train, y_train, X_val, y_val, n_epochs, time=False):
    ...
    for i in learning_rate:
        ...
        train_loss_history, val_loss_history, epoch = train(model, X_train, y_train, X_val, y_val, n_epochs, lf,
...
if __name__ == '__main__':
    ...
    # data initialization
    X_train, y_train, X_test, y_test, X_val, y_val = load_gpu_data_with_batches(data, test_size=test_size,
                                                                                cv_size=cv_size, seq_length=seq)
    # print(y_train)
    X_train, y_train, X_test, y_test, X_val, y_val = X_train.to(device), y_train.to(device), X_test.to(
        device), y_test.to(device), X_val.to(device), y_val.to(device)
    ...
    model = ParametricCNNLSTM(num_layers_conv, output_channels, kernel_sizes, stride_sizes, padding_sizes,
                              hidden_size_lstm, num_layers_lstm, hidden_neurons_dense).double()
...
# Define the range of alpha and lambda values to test
alpha_range = np.linspace(0, 1, 101)
lambda_range = np.linspace(0, 1, 101)

# Calculate the number of true positives, false positives, true negatives, and false negatives for each alpha and lambda value
tp = np.zeros((len(alpha_range), len(lambda_range)))
fp = np.zeros((len(alpha_range), len(lambda_range)))
tn = np.zeros((len(alpha_range), len(lambda_range)))
fn = np.zeros((len(alpha_range), len(lambda_range)))

for i, alpha in enumerate(alpha_range):
    for j, lambda_ in enumerate(lambda_range):
        for k in range(len(pred)):
            if abs(pred[k] - true[k]) <= alpha and true[k] >= lambda_:
                tp[i, j] += 1
            elif abs(pred[k] - true[k]) <= alpha and true[k] < lambda_:
                fp[i, j] += 1
            elif abs(pred[k] - true[k]) > alpha and true[k] >= lambda_:
                fn[i, j] += 1
            elif abs(pred[k] - true[k]) > alpha and true[k] < lambda_:
...
# Calculate the accuracy, precision, F1 score, and AUC for each alpha and lambda value
accuracy = (tp + tn) / (tp + fp + tn + fn)
precision = tp / (tp + fp)
recall = tp / (tp + fn)
f1 = 2 * (precision * recall) / (precision + recall)
auc = np.trapz(f1, lambda_range)

# Plot the alpha-lambda plot
plt.imshow(f1, origin='lower', extent=[lambda_range.min(), lambda_range.max(), alpha_range.min(), alpha_range.max()],
           aspect='auto', cmap='coolwarm')
plt.colorbar()
plt.xlabel('Lambda')
plt.ylabel('Alpha')
plt.title('Alpha-Lambda Plot (AUC = {:.3f})'.format(auc))
plt.show()
